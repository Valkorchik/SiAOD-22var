...
#include <iostream>
using namespace std;
void MatrixCreate(double** matrix, int order)
{
...
void RandomMatrixFiller(double** matrix, int order)
{
	srand(time(NULL));
	for(int i=0;i<order;i++)
	{
...
void MatrixShower(double** matrix,int order)
{
	cout<<"=== Matrix ===\n";
	for(int i=0;i<order;i++)
	{
...
void InverseMatrixShower(double** ematrix,int order)
{
	cout<<"=== Inverse Matrix ===\n";
	for(int i=0;i<order;i++)
	{
...
void EdinCreate(double** ematrix, int order)
{
	for (int i=0; i<order; i++)
	{
		for (int j=0; j<order; j++)
...
void TransformMatrix(double** matrix, double** ematrix,int order)
{
	int j=0, m, n;
	do
...
int main()
{
	...
	double** ematrix=new double* [order];
	for (int i = 0; i < order; i++)
	{
		matrix[i] = new double[order];
		ematrix[i] = new double[order];
	}
